...
export async function grantBreakGlassAction(form: FormData): Promise<void> {
  ...
  const companyId = z.uuid().safeParse(text(form, "companyId"));
  if (!z.uuid().safeParse(accountId).success) return back("/accounts", "Invalid request");
  if (!companyId.success) return back(path, "Choose the company to open");
  let pending = false;
  try {
    const r = await grantBreakGlass(db(), keyWrapper(), {
      adminId: admin.adminId,
      ip: admin.ip,
      accountId,
    ...
    path,
    pending
      ? "Request recorded; another admin must approve it before access starts"
      : "Access granted; the account holder has been notified",
  );
}

export async function approveBreakGlassAction(form: FormData): Promise<void> {
  const admin = await requireAdmin();
  const accountId = text(form, "accountId");
  const path = `/accounts/${accountId}`;
  const grantId = z.uuid().safeParse(text(form, "grantId"));
  if (!grantId.success) return back(path, "Invalid request");
  try {
    await approveBreakGlass(db(), keyWrapper(), {
      adminId: admin.adminId,
      ip: admin.ip,
    ...
    return back(path, message(error, "Could not approve"));
  }
  done(path, "Approved; access has started and the account holder has been notified");
}
