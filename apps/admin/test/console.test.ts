...
async function customer() {
  ...
  const c = await pool().query<{ id: string }>(`insert into companies (account_id, name) values ($1, 'Synthetic Traders') returning id`, [
    accountId,
  ]);
...
describe("routing and registry", () => {
  it("publishes a new routing version that keeps the active prompt, and refuses unknown models", async () => {
    ...
    await pool().query(`update tier_routing set prompt_version = 3 where tier = 'efficient' and stage = 'commentary'`);
    const r = await publishRoute(pool(), {
      adminId,
      ip: null,
      route: { tier: "efficient", stage: "commentary", modelId: "claude-sonnet-5", effort: "low", maxTokens: 6000, fallbackChain: [] },
    });
    const row = (await listRouting(pool())).find((x) => x.tier === "efficient" && x.stage === "commentary");
    expect(row).toMatchObject({ version: r.version, max_tokens: 6000, effort: "low", prompt_version: 3 });
    await expect(
      publishRoute(pool(), {
        adminId,
        ip: null,
        route: { tier: "efficient", stage: "commentary", modelId: "gpt-9", effort: null, maxTokens: 1000, fallbackChain: [] },
      }),
  ...
  it("re-verifies model prices as a new registry version", async () => {
    ...
    expect(Date.now() - (latest.rows[0]?.verified_at.getTime() ?? 0)).toBeLessThan(60_000);
  });
});
...
describe("config editor", () => {
  it("publishes a new version with the same shape and refuses shape changes and locked keys", async () => {
    const adminId = await admin();
    const r = await publishConfig(pool(), { adminId, ip: null, key: "chat.max_rounds", valueJson: "4" });
    expect((await listConfig(pool())).find((c) => c.key === "chat.max_rounds")).toMatchObject({ value: 4, version: r.version });
    await expect(publishConfig(pool(), { adminId, ip: null, key: "chat.max_rounds", valueJson: '"four"' })).rejects.toThrow(/stay a number/u);
    await expect(
      publishConfig(pool(), { adminId, ip: null, key: "ratelimit.limits", valueJson: '{"ai_per_account": 1}' }),
    ).rejects.toThrow(/same fields/u);
    await expect(publishConfig(pool(), { adminId, ip: null, key: "unknown.key", valueJson: "1" })).rejects.toThrow(/not editable/u);
    await publishConfig(pool(), { adminId, ip: null, key: "chat.max_rounds", valueJson: "5" });
  });
});
...
describe("jobs inspector", () => {
  it("shows the state timeline, AI calls and ledger, and only the names of checkpoint keys", async () => {
    ...
    await grantCredits(pool(), { accountId: c.accountId, credits: 5_000n, source: "admin_grant", idempotencyKey: randomUUID() });
    const job = await createJob(pool(), {
      ...
      size: { files: 0, sheets: 0, columns: 0, rows: 0, distinctLedgerValues: 0, referenceMisSheets: 0 },
    });
...
describe("library curation", () => {
  it("approves a candidate into the library, attributed to the admin, once", async () => {
    const adminId = await admin();
    const head = await pool().query<{ id: string }>(`select id from mis_heads where code = 'OPEX'`);
    ...
    await decideCandidate(pool(), { adminId, ip: null, candidateId: id, decision: "approved" });
    const entry = await pool().query(
      `select source, promoted_by_admin_id, tenant_count from global_mapping_library where normalized_name = 'courier charges'`,
    );
    expect(entry.rows[0]).toEqual({ source: "promoted", promoted_by_admin_id: adminId, tenant_count: 7 });
    await expect(decideCandidate(pool(), { adminId, ip: null, candidateId: id, decision: "rejected" })).rejects.toThrow(/already decided/u);
  });
});
...
describe("break-glass", () => {
  it("needs a reason and a bounded time, notifies the holder, audits every view, and ends on revoke", async () => {
    ...
    await storeSnapshot(pool(), wrapper, {
      ...
      payload: {
        ...
        metricStore: {
          ...snap.metricStore,
          values: [
            { metricId: "revenue", period: snap.period, dims: {}, value: "100", nullReason: null, unit: "paise", formula: "", inputs: [] },
          ],
        },
      },
    });

    await expect(grantBreakGlass(pool(), { adminId, ip: null, accountId: c.accountId, reason: "help", minutes: 10 })).rejects.toThrow(
      /at least 20 characters/u,
    );
    await expect(
      grantBreakGlass(pool(), { adminId, ip: null, accountId: c.accountId, reason: "Customer asked us to check a mapping", minutes: 600 }),
    ...
    const notice = await pool().query(`select type from notifications where account_id = $1 and type = 'security.break_glass'`, [c.accountId]);
    expect(notice.rowCount).toBe(1);
    expect(await activeGrants(pool(), c.accountId)).toHaveLength(1);

    const view = await breakGlassView(pool(), wrapper, { adminId, ip: null, grantId, companyId: c.companyId });
    ...
    await expect(breakGlassView(pool(), wrapper, { adminId: other, ip: null, grantId, companyId: c.companyId })).rejects.toThrow(
      /active break-glass grant/u,
    );
    await expect(
      breakGlassView(pool(), wrapper, { adminId, ip: null, grantId, companyId: c.companyId, now: new Date(Date.now() + 16 * 60_000) }),
    ).rejects.toThrow(/active break-glass grant/u);
    await revokeBreakGlass(pool(), { adminId, ip: null, grantId });
    await expect(breakGlassView(pool(), wrapper, { adminId, ip: null, grantId, companyId: c.companyId })).rejects.toThrow(
      /active break-glass grant/u,
    );
    ...
    expect(audit.rows.map((r) => r.action)).toEqual(["admin.break_glass_granted", "admin.break_glass_viewed", "admin.break_glass_revoked"]);
