...
const bodySchema = z.object({ action: z.literal("undo"), baseVersion: z.number().int().positive() });
  ...
  return withAccount(async (account) => {
    ...
    try {
      return await idempotent(request, `template-undo:${account.accountId}:${id}`, parsed.raw, async () => {
        ...
        return { status: 200, body: { blueprintVersion: r.blueprintVersion, canUndo: r.canUndo } };
      });
    } catch (error) {
