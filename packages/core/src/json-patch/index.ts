/**
 * RFC 6902 JSON Patch over RFC 6901 JSON Pointers, applied immutably and then validated against a
 * schema (SPEC §24.2, §27). Dashboard and template edits happen only this way, from UI controls or
 * `chat_edit`. Keys that could reach object prototypes are refused outright.
 */
...
function arrayIndex(arr: Json[], token: string, index: number, forAdd: boolean): number {
  ...
  if (!/^(0|[1-9]\d*)$/u.test(token))
    throw new PatchError(index, `"${token}" is not an array index`);
  const i = Number.parseInt(token, 10);
  if (i > arr.length || (!forAdd && i === arr.length))
    throw new PatchError(index, `index ${token} is out of bounds`);
  return i;
}
...
function get(doc: Json, path: string[], index: number): Json {
  ...
  if (Array.isArray(parent))
    return parent[arrayIndex(parent, last, index, false)] ?? null;
...
function add(doc: Json, path: string[], value: Json, index: number): Json {
  ...
  const last = path[path.length - 1] ?? "";
  if (Array.isArray(parent))
    parent.splice(arrayIndex(parent, last, index, true), 0, value);
...
function remove(doc: Json, path: string[], index: number): Json {
  ...
  else if (isObject(parent) && Object.hasOwn(parent, last))
  ...
  operations.forEach((op, i) => {
    switch (op.op) {
      ...
      case "replace": {
        ...
        get(doc, path, i);
        doc = add(
          path.length === 0 ? doc : remove(doc, path, i),
          path,
          clone(op.value),
          i,
        );
      ...
      case "move": {
        ...
        if (op.path.startsWith(`${op.from}/`))
          throw new PatchError(i, "cannot move a value into itself");
      ...
      case "copy":
        ...
        break;
      case "test":
        if (!equal(get(doc, tokens(op.path, i), i), op.value))
          throw new PatchError(i, "test failed");
        break;
    }
  });
  return doc;
...
/** Validates the operations, applies them, and validates the result against `schema`. */
export function patchDocument<S extends z.ZodType>(
  document: unknown,
  operations: unknown,
  schema: S,
): PatchResult<z.infer<S>> {
  const ops = patchSchema.safeParse(operations);
  if (!ops.success)
    return {
      ok: false,
      errors: ops.error.issues.map((i) => `${i.path.join(".")}: ${i.message}`),
    };
  let next: unknown;
  try {
    next = applyJsonPatch(document, ops.data);
  } catch (error) {
    return {
      ok: false,
      errors: [error instanceof Error ? error.message : "patch failed"],
    };
  }
