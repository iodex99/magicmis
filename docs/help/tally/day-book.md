...
Voucher-level detail for the period.
...
- Period: the full month. A month split across two files is fine: load both.
- Show every ledger line of each voucher (not only the first).
...
- Date, voucher type and number printed only on a voucher's first line are carried to its other lines.
