...
|---|---|---|---|
| R-01 | **Product name.** Spec says `[PRODUCT_NAME]`; directory is `magicmis`. Note §32 forbids "magic" wording in customer UI, which sits awkwardly with MagicMIS as a brand. Held as a single constant (`packages/core/src/brand.ts`, used by web, admin and email), currently the placeholder "MIS Studio", so the real name is a one-line change. It also decides the apex domain, and with it whether to submit that domain to the **HSTS preload list** — near-irreversible, so it waits for the name (ADR 0029 §2). | Before launch | open |
...
| R-17 | **Google / Microsoft sign-in** — §8 says may be offered, decision recorded in ADR. | Phase 1 | **closed** — not offered in this build; email and password only (TOTP was removed by ADR 0028). Reversible without a schema change. See ADR 0009. |
