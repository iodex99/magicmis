...
| P5 | Uploaded source files (ADR 0032) | Everything in the uploaded files, including third-party names, PAN, Aadhaar, bank details | Build the MIS and answer chat questions the user asked for | Contract (the account holder is responsible for their clients' data) | Output store (Supabase Storage), each chunk sealed under the company key; parsed in server memory during a run | `sources.retention_days` after upload (R-63), sooner on user deletion; crypto-shredded with the company | `packages/jobs/src/sources.ts`, `apps/web/src/lib/server/run-job.ts` |
