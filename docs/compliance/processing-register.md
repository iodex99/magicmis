...
| P5 | Uploaded source files (ADR 0032) | Everything in the uploaded files, including third-party names, PAN, Aadhaar, bank details | Build the MIS and answer chat questions the user asked for | Contract (the account holder is responsible for their clients' data) | Output store (Supabase Storage), each chunk sealed under the company key; parsed in server memory during a run | Until the user deletes it (ADR 0047: `sources.retention_days` = 0, no expiry; R-63), or for that many days if an operator sets a period; crypto-shredded with the company. Every decryption is recorded in `source_upload_reads` (purpose and time, no content), visible to the account; no staff-facing path opens a file | `packages/jobs/src/sources.ts`, `apps/web/src/lib/server/run-job.ts` |
